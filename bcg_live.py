#!/usr/bin/env python3
"""
Upgraded BCG Live Rolling Dashboard (With Simulation Overrides)
...
def main():
    ...
    fig.canvas.manager.set_window_title("Live 3-Axis Ballistocardiography (BCG) Simulation Monitor Console")
    
    # Grid specification adjustment to lease space for simulation buttons down at the footer layout region
    ax_raw = plt.subplot2grid((20, 2), (0, 0), rowspan=9, colspan=1)
    ax_filt = plt.subplot2grid((20, 2), (10, 0), rowspan=9, colspan=1)
    ax_fft = plt.subplot2grid((20, 2), (0, 1), rowspan=9, colspan=1)
    ax_panel = plt.subplot2grid((20, 2), (10, 1), rowspan=9, colspan=1)
    ...
    ax_panel.axis('off')
    panel_text = ax_panel.text(0.05, 0.5, "", fontsize=11, color='#ffffff', va='center', fontfamily='monospace')
    
    # Callback Handlers to pass simulation characters down to your ESP32
    ...
    # Construct Control Buttons on Figure Baseline Canvas 
    ax_btn_live = plt.axes([0.15, 0.015, 0.18, 0.035])
    ax_btn_brady = plt.axes([0.40, 0.015, 0.20, 0.035])
    ax_btn_tachy = plt.axes([0.68, 0.015, 0.20, 0.035])

    btn_live = Button(ax_btn_live, 'Resume Live Data', color='#27ae60', hovercolor='#2eccc7')
    ...
    def update_plot(frame):
        ...
        dist = int(0.22 * fs)  # Scale distance constraint to capture up to 180+ BPM during tachycardia
        ...
        if len(bpm_history) >= 5:
            ...
            bpm_std = 0.0
            stability_flag = "Calculating Profile..."
        ...
        panel_content = (
            f"===================================================\n"
            f"             LIVE MONITOR SYSTEM DIAGNOSTICS      \n"
            f"===================================================\n"
            f"Sampling Track Clock Rate  : {fs:.2f} Hz\n\n"
            f"Observed Spectral Peaks (FFT Estimates):\n"
            f"  - AX Channel BPM         : {fft_data['ax']['bpm']:.1f}\n"
            f"  - AY Channel BPM         : {fft_data['ay']['bpm']:.1f}\n"
            f"  - AZ Channel BPM         : {fft_data['az']['bpm']:.1f}\n"
            f"  - Computed Magnitude BPM : {fft_data['magnitude']['bpm']:.1f}\n\n"
            f"  - Peak-Detection BPM     : {peak_bpm:.1f} (Best Channel Extraction)\n\n"
            f"Signal Integrity Metrics:\n"
            f"  - Selected Extraction Target : {best_channel.upper()}\n"
            f"  - Quality Index Score (SQS)  : {quality_metrics[best_channel]['sqs']:.2f}\n"
            f"  - Process Confidence Rating  : {confidence_score:.1f} %\n"
            f"  - Rhythm Classification      : {stability_flag}\n"
            f"  - Running Historical Average : {running_avg_bpm:.1f} BPM\n"
            f"===================================================\n"
        )
